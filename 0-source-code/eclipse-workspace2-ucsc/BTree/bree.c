...
void display(link h, int height) {
	int j, k;
	// if at data node, display all the keys and items in the node
	if (height == 0) {
		printf("\n Data node ");
		printf("%p  : ", h); // display link
		printf("size %d ", h->m);

		for (j = 0; j < h->m; j++) {
			printf("key %d  ", h->b[j].key); // display key
			printf("item %d  ", h->b[j].ref.item.key); // display item's key
			printf("%f  ", h->b[j].ref.item.value1); // display item's value
			printf("%f , ", h->b[j].ref.item.value2); // display item's value
	...
	if (height != 0) {
		...
		for (j = 0; j < h->m; j++) {
			...
			if (j == 0) {
				...
				// display all the keys and links in this index node
				for (k = 0; k < h->m; k++) {
					printf("key %d  ", h->b[k].key); // display key
					printf("next %p , ", h->b[k].ref.next); // display pointer to child
				}
			}

			// at last item or next item has greater key
			display(h->b[j].ref.next,  height - 1);
		}
